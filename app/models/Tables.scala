...
trait Tables {
  ...
  case class UsersRow(rowId: Long, email: String, password: String, fullname: String, isadmin: Boolean)
  ...
  class Users(_tableTag: Tag) extends Table[UsersRow](_tableTag, "users") {
    def * = (rowId, email, password, fullname, isadmin) <> (UsersRow.tupled, UsersRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(rowId), Rep.Some(email), Rep.Some(password), Rep.Some(fullname), Rep.Some(isadmin)).shaped.<>({r=>import r._; _1.map(_=> UsersRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
