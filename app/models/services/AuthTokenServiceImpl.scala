...
class AuthTokenServiceImpl @Inject() (authTokenDAO: AuthTokenDAO, clock: Clock) extends AuthTokenService {
   ...
   *
   * @param accountId The account ID for which the token should be created.
   * @param expiry The duration a token expires.
   * @return The saved auth token.
   */
  def create(accountId: UUID, expiry: FiniteDuration = 5 minutes) = {
    val token = AuthToken(UUID.randomUUID(), accountId, clock.now.withZone(DateTimeZone.UTC).plusSeconds(expiry.toSeconds.toInt))
